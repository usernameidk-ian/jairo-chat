...
const adminDrawMenu = document.getElementById('admin-draw-menu');

// --- ERASER FEATURE (Dynamically injected into your tools bar) ---
const eraserBtn = document.createElement('button');
eraserBtn.textContent = '🧹 Eraser: OFF';
eraserBtn.style.padding = '5px 10px';
eraserBtn.style.cursor = 'pointer';
eraserBtn.style.background = '#2f3136';
eraserBtn.style.color = '#fff';
eraserBtn.style.border = '1px solid #4f545c';
...
if (drawToggleBtn) {
    drawToggleBtn.addEventListener('click', () => {
        ...
        if (isDrawingMode) {
            ...
            drawToggleBtn.style.background = '#007bff';
            drawTools.style.display = 'flex';
            drawingLayer.style.pointerEvents = 'auto'; // allow drawing
        } else {
            drawToggleBtn.classList.remove('active');
            drawToggleBtn.style.background = '';
            drawTools.style.display = 'none';
            drawingLayer.style.pointerEvents = 'none'; // pass clicks thru
            isErasing = false;
            eraserBtn.textContent = '🧹 Eraser: OFF';
            eraserBtn.style.background = '#2f3136';
        }
...
eraserBtn.addEventListener('click', () => {
    ...
    eraserBtn.style.background = isErasing ? '#ed4245' : '#2f3136';
});

// Start Drawing Path
drawingLayer.addEventListener('mousedown', (e) => {
    if (!isDrawingMode || isErasing) return;
    isDrawing = true;
    
    // Scale coords for responsive resize
    ...
    currentPathData = `M ${lastX} ${lastY}`;
    const newDrawRef = drawingsRef.push();
    currentPathId = newDrawRef.key;
    ...
    newDrawRef.set({
        ...
        username: username,
        fingerprint: deviceID,
        timestamp: Date.now() // Timers added here
    });
});

// Update Drawing Path
drawingLayer.addEventListener('mousemove', (e) => {
    if (!isDrawingMode || !isDrawing || isErasing) return;
    const rect = drawingLayer.getBoundingClientRect();
    const scaleX = 1920 / rect.width;
    const scaleY = 1080 / rect.height;
    ...
    drawingsRef.child(currentPathId).update({ path: currentPathData });
});

// End Drawing Path and set 3-Minute Deletion Timer
window.addEventListener('mouseup', () => {
    if (isDrawing) {
        isDrawing = false;
        
        // Timer: Drawing will auto-delete in 3 minutes (180,000 milliseconds)
        const drawnId = currentPathId;
        setTimeout(() => {
            drawingsRef.child(drawnId).remove();
        }, 3 * 60 * 1000); 
...
// Render Drawings on Screen
drawingsRef.on('child_added', (snapshot) => {
    const data = snapshot.val();
    const key = snapshot.key;
    
    // Safety check: if drawing is older than 3 minutes upon load, instantly delete it
    ...
    pathEl.setAttribute('stroke-width', data.thickness);
    pathEl.setAttribute('fill', 'none');
    pathEl.setAttribute('stroke-linecap', 'round');
    pathEl.setAttribute('stroke-linejoin', 'round');
    pathEl.style.pointerEvents = 'stroke';

    // Hover Tooltip & Eraser Drag
    pathEl.addEventListener('mouseenter', (e) => {
        if (isDrawingMode && !isDrawing) {
            drawTooltip.textContent = data.username;
            drawTooltip.style.display = 'block';
            drawTooltip.style.left = e.pageX + 10 + 'px';
            drawTooltip.style.top = e.pageY + 10 + 'px';
        }
        
        // Eraser drag mechanic
        if (isDrawingMode && isErasing && (e.buttons === 1)) {
             if (data.fingerprint === deviceID || isAdmin) {
                 drawingsRef.child(key).remove();
             }
        }
    });

    // Eraser Click Mechanic
    pathEl.addEventListener('mousedown', (e) => {
         if (isDrawingMode && isErasing) {
             if (data.fingerprint === deviceID || isAdmin) {
                 drawingsRef.child(key).remove();
             } else {
                 alert("You can only erase your own drawings!");
             }
         }
    });
    ...
    pathEl.addEventListener('contextmenu', (e) => {
        if (!isAdmin) return; // FIXED: strictly stops non-admins from triggering this menu
        
        e.preventDefault();
        adminDrawMenu.style.display = 'flex';
        // FIXED: Sets position to where the mouse actually clicked
        adminDrawMenu.style.left = e.pageX + 'px';
        adminDrawMenu.style.top = e.pageY + 'px';

        document.getElementById('del-draw-btn').onclick = () => {
            drawingsRef.child(key).remove();
            adminDrawMenu.style.display = 'none';
        };

        document.getElementById('time-draw-btn').onclick = () => {
            const duration = prompt(`How many seconds to timeout drawer (${data.username})?`);
            if (duration && !isNaN(duration)) {
                const untilTime = Date.now() + (parseInt(duration) * 1000);
                db.ref("timeouts").child(data.fingerprint).set({ 
                    until: untilTime,
                    originalName: data.username 
                });
                drawingsRef.child(key).remove(); // Clears their drawing too
                alert(`Timed out ${data.username} for ${duration}s`);
            }
            adminDrawMenu.style.display = 'none';
        };
    });

    drawingLayer.appendChild(pathEl);
    activePathElements[key] = pathEl;
});

// Update lines live for other users
drawingsRef.on('child_changed', (snapshot) => {
    const data = snapshot.val();
    const key = snapshot.key;
    if (activePathElements[key]) {
        activePathElements[key].setAttribute('d', data.path);
    }
});

// Remove line from screen if deleted from Firebase
drawingsRef.on('child_removed', (snapshot) => {
    const key = snapshot.key;
    if (activePathElements[key]) {
        activePathElements[key].remove();
        delete activePathElements[key];
    }
});

// Ensure Admin menu closes if you click away
document.addEventListener('click', (e) => {
    if (isAdmin && adminDrawMenu.style.display === 'flex' && !adminDrawMenu.contains(e.target)) {
        adminDrawMenu.style.display = 'none';
    }
});

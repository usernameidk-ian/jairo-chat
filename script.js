...
// keep raw and trimmed variations for security checks
...
// impersonation check: prevents people from named " b i a n "
...
if (trimmed === adminUsername) {
  ...
  if (password === adminPassword) {
    isAdmin = true;
    username = adminUsername; // normalize to exact canonical admin
  } else {
...
// For DB/timeouts we use cleanName (normalized) as the identity key
const identityKey = cleanName || username.replace(/\s+/g, "").toLowerCase();

// ======================================================
// 2. HELPER FUNCTIONS (Colors & Icons)
// ======================================================
...
// show admin clear button if admin is logged in
...
// ======================================================
// 5. MESSAGE RECEIVING & MEDIA RENDERING
// ======================================================
messagesRef.on("child_added", (snapshot) => {
  ...
  if (msg && msg.text) {
    ...
    // Container for text or images/videos
    const contentContainer = document.createElement("div");
    contentContainer.className = "msg-content";

    const trimmedText = msg.text.trim().toLowerCase();

    // IMAGE DETECTION: checks if link ends in jpg, png, gif, or webp
    if (trimmedText.match(/\.(jpeg|jpg|gif|png|webp)$/) != null) {
      const img = document.createElement("img");
      img.src = msg.text.trim(); 
      img.className = "chat-media";
      contentContainer.appendChild(img);
    } 
    // VIDEO DETECTION: checks if link ends in mp4, webm, or ogg
    else if (trimmedText.match(/\.(mp4|webm|ogg)$/) != null) {
      const video = document.createElement("video");
      video.src = msg.text.trim();
  ...
  // Admin Controls (Delete & Timeout buttons)
  if (isAdmin) {
    ...
    timeoutBtn.className = "admin-btn";
...
// Remove message from screen if deleted in Firebase
...
// Resume music on first user click (browser requirement)
